...
mod tests {
    ...
    fn test_add_fp() {
        ...
        const POINT_SOURCE: [i32; 8] = [1, 5, 4, 10, 234, 1024, 817, 912];
        let mut points = Vec::new();
        for i in (0..POINT_SOURCE.len()).step_by(2) {
            let x = BigInteger::with_i32(POINT_SOURCE[i]);
            let y = BigInteger::with_i32(POINT_SOURCE[i + 1]);
            let point = curve.create_point_x_y(Some(x), Some(y)).unwrap();
            points.push(point);
        }

        //assert_eq!(points[0].add(&points[1]), points[2], "p0 plus p1 does not equal p2");
