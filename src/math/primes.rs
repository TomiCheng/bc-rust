...
pub fn generate_st_random_prime(
    ...
    if length < 2 || length > 65536 {
        return Err(BcError::with_invalid_argument("length must be between 2 and 65536"));
    }

    if input_seed.is_empty() {
        return Err(BcError::with_invalid_argument("input_seed cannot be empty"));
    }
...
///
pub fn enhanced_mr_probable_prime_test<TRngCore: RngCore>(
    ...
    if iterations < 1 {
        return Err(BcError::with_invalid_argument("iterations must be at least 1"));
    }
...
pub fn is_mr_probable_prime_to_base(
    ...
    if base_value >= &candidate.subtract(&(*ONE)) {
        return Err(BcError::with_invalid_argument("base_value must be < candidate-1"));
    }
    ...
    if length < 33 {
        ...
        loop {
            ...
            if prime_gen_counter > (4 * length) {
                return Err(BcError::with_invalid_operation("Too many iterations in generation of prime number"));
            }
        }
    }
...
fn hash(d: &mut dyn Digest, input: &[u8], output: &mut [u8]) -> Result<()> {
...
fn hash_gen(d: &mut dyn Digest, seed: &mut [u8], count: u32) -> Result<BigInteger> {
    let d_len = d.get_digest_size();
...
mod tests {
    ...
    use crate::math::big_integer::TWO;
    use crate::math::BigInteger;
    use crate::math::primes::enhanced_mr_probable_prime_test;
    use crate::math::primes::generate_st_random_prime;
    use crate::math::primes::has_any_small_factors;
    use crate::math::primes::is_mr_probable_prime;
    use crate::math::primes::is_mr_probable_prime_to_base;
    use crate::math::primes::SMALL_FACTOR_LIMIT;
    use crate::util::big_integers::create_random_in_range;
    use crate::Result;
        ...
        for digest in digests.iter_mut() {
            ...
            while iterations < ITERATIONS {
                ...
                let st = match generate_st_random_prime(digest.as_mut(), PRIME_BITS as u32, &input_seed)
                {
                    Ok(v) => v,
                    Err(e) => {
                        if e.error().to_string().starts_with("Too many iterations") {
                            iterations -= 1;
                            continue;
                        }
                        panic!("Unexpected error: {}", e.error().to_string());
                    }
                };
