...
fn random_prime() -> BigInteger {
    ...
    BigInteger::with_random_certainty(PRIME_BITS, PRIME_CERTAINTY as i32, &mut random)
}
...
fn reference_is_mr_probable_prime(x: &BigInteger, num_bases: u32) -> bool {
    ...
    for _ in 0..num_bases {
        ...
        if is_mr_probable_prime_to_base(&x, &b) {
            return false;
        }
    }
    true
...
fn is_prime(x: &BigInteger) -> bool {
    x.is_probable_prime(PRIME_CERTAINTY as i32)
}
...
fn test_enhanced_mr_probable_prime() {
    ...
    for iterations in 0..ITERATIONS {
        let prime = random_prime();
        let mr = enhanced_mr_probable_prime_test(&prime, &mut random, mr_interations);
        ...
        let mr2 = enhanced_mr_probable_prime_test(&prime_power, &mut random, mr_interations);
        ...
        let mr3 = enhanced_mr_probable_prime_test(&non_prime_power, &mut random, mr_interations);
...
fn test_mr_probable_prime() {
    ...
    for _ in 0..ITERATIONS {
        let prime = random_prime();
        assert!(is_mr_probable_prime(&prime, &mut random, mr_iterations));

        let non_prime = random_prime().multiply(&prime);
        assert!(!is_mr_probable_prime(
            &non_prime,
            &mut random,
            mr_iterations
        ));
...
fn test_st_random_prime() {
    ...
    for digest in digests.iter_mut() {
        ...
        for iterations in 0..ITERATIONS {
            ...
            let st = generate_st_random_prime(digest.as_mut(), PRIME_BITS as u32, &input_seed);
            assert!(is_prime(st.get_prime()));

            let st2 = generate_st_random_prime(digest.as_mut(), PRIME_BITS as u32, &input_seed);
            ...
            let st3 = generate_st_random_prime(digest.as_mut(), PRIME_BITS as u32, &input_seed);
