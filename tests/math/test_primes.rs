...
fn test_enhanced_mr_probable_prime() {
    ...
    for iterations in 0..ITERATIONS {
        ...
        let mr3 = enhanced_mr_probable_prime_test(&non_prime_power, &mut random, mr_interations).unwrap();
...
fn test_st_random_prime() {
    ...
    for digest in digests.iter_mut() {
        ...
        for iterations in 0..ITERATIONS {
            ...
            let st = generate_st_random_prime(digest.as_mut(), PRIME_BITS as u32, &input_seed).unwrap();
            assert!(is_prime(st.get_prime()).unwrap());

            let st2 = generate_st_random_prime(digest.as_mut(), PRIME_BITS as u32, &input_seed).unwrap();
            ...
            let st3 = generate_st_random_prime(digest.as_mut(), PRIME_BITS as u32, &input_seed).unwrap();
